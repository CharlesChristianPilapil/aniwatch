...
const SideBar = ({ isToggled = false, onToggle }: SideBarType) => {
    ...
    return isToggled ? (
        <aside
            ...
            className="fixed top-0 left-0 w-full h-full bg-background/25 backdrop-blur-xs z-10"
        >
            <div
            ...
            >
                <button
                    onClick={() => onToggle(false)}
                    className="cursor-pointer px-5 mb-5 hover:text-primary-accent focus:text-primary-accent focus:underline focus:outline-none"
                >
                    Close Menu
                </button>
                <ul>
                    {links.map((link, index) => (
                        <li key={index} className="w-full">
                            <Link
                                to={link.url}
                                className="px-5 py-5 border-b border-main/25 w-full block hover:text-secondary-accent hover:bg-background/50 focus:text-secondary-accent focus:bg-background/50 outline-secondary-accent"
                                onClick={() => onToggle(false)}
                            >
                                {link.label}
                            </Link>
